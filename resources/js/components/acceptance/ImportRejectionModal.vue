    ...
    >
      ...
      <div class="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-5">
        ...
        >
          ...
          <div
            v-if="selectedType === 'load_with_trips'"
            class="rounded-md border p-3 bg-muted/10 space-y-1"
          >
            <p
            ...
            <p class="text-xs text-muted-foreground">
              You must upload a Trips CSV alongside the Loads CSV to import with driver
              names.
            </p>
            <label
              class="cursor-pointer inline-flex items-center gap-2 text-sm text-primary hover:underline mt-1"
            >
              <Icon name="file-plus" class="h-4 w-4" />
              {{ tripsFile ? tripsFile.name : "Attach Trips CSV" }}
              <input
                ref="tripsFileInput"
                type="file"
                class="hidden"
          ...
          >
            This will upload a Trips CSV only to update driver names on existing load
            rejections. No new rejections will be created.
          </div>
        ...
        <template v-if="validationResults">
          ...
          <div
            v-if="isSuperAdmin && selectedTenantId"
            class="rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-xs text-primary flex items-center gap-2"
          >
            <Icon name="building" class="h-4 w-4 flex-shrink-0" />
            Importing for:
            <span class="font-semibold ml-1">{{ selectedTenantName }}</span>
          </div>
          ...
          <div v-if="validationResults.headers?.length" class="rounded-lg border p-3">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-xs text-muted-foreground"
                >{{ validationResults.headers.length }} columns</span
              >
            </div>
          ...
          <div
            ...
            v-if="validationResults.summary.total !== -1"
          >
            <Card class="border-2">
          ...
          >
            <AlertTitle class="text-amber-700 dark:text-amber-300"
              >Trips Only Mode</AlertTitle
            >
            <AlertDescription class="text-amber-600 dark:text-amber-400">
              Only driver names will be updated on existing load rejections. No new
          ...
          <div v-if="validationResults.valid?.length && !validationResults.trips_only">
            <h3
              class="text-base font-semibold text-green-600 flex items-center gap-2 mb-3"
            >
            ...
            <div class="border rounded-lg overflow-hidden">
              <Table>
                ...
                <TableBody>
                  <TableRow
                    v-for="row in validationResults.valid.slice(0, 5)"
                    :key="row.rowNumber"
                  >
      ...
      <!-- Footer -->
      <div
        class="border-t px-4 sm:px-6 py-4 flex items-center justify-between gap-3 bg-background"
      >
        <div class="text-xs text-muted-foreground">
          ...
          <span
            v-if="selectedType"
            :class="{ 'ml-3': isSuperAdmin && selectedTenantName }"
          >
            Mode: <span class="font-medium">{{ currentOptionLabel }}</span>
          </span>
        </div>
        <div class="flex gap-3">
          <Button @click="handleClose" variant="outline" :disabled="isImporting"
            >Close</Button
          >
          <Button
            v-if="canImport"
...
const importOptions = [
  ...
  },
  {
    ...
    label: "Load + Trips",
    description:
      "Import rejected loads and attach a Trips CSV to auto-fill driver names.",
  },
  {
    value: "trips_only",
    label: "Trips Only",
...
const selectedTenantName = computed(() => {
  ...
  return (
    (props.tenants as any[]).find((t) => t.id === selectedTenantId.value)?.name ?? ""
  );
});
...
const canImport = computed(() => {
  ...
  return (
    (validationResults.value.summary?.valid ?? 0) > 0 &&
...
const ROUTE_MAP: Record<string, { validate: string; confirm: string }> = {
  ...
  load: {
    validate: "acceptance.validateLoadImport",
    confirm: "acceptance.confirmLoadImport",
  },
  load_with_trips: {
    validate: "acceptance.validateLoadImport",
    confirm: "acceptance.confirmLoadImport",
...
function onTripsFileChange(e: Event) {
  ...
  // 🚨 If main file already selected, auto-validate now
  if (selectedType.value === "load_with_trips" && selectedFile.value && tripsFile.value) {
    validateFiles();
  }
}
...
function triggerValidation(file: File) {
  ...
  if (!isCsv) {
    emit("error", "Please upload a valid CSV file.");
    return;
  }

  selectedFile.value = file;

  // 🚨 NEW: If load_with_trips, require trips file first
  if (selectedType.value === "load_with_trips" && !tripsFile.value) {
    emit("error", "Please attach the required Trips CSV file.");
    return;
  }
...
function validateFiles() {
  ...
  if (selectedType.value === "load_with_trips") {
    if (!tripsFile.value) {
      emit("error", "Trips CSV is required.");
      isValidating.value = false;
      return;
    }
  ...
  if (selectedType.value === "trips_only") {
    data.delete("file");
    data.append("trips_file", selectedFile.value);
  }
...
function confirmImport() {
  ...
  router.post(getRoute("confirm"), payload, {
    ...
    onSuccess: () => {
      ...
      emit(
        "success",
        `Successfully imported ${count} rejection(s) for ${
          selectedTenantName.value || "company"
        }.`
      );
      handleClose();
    },
...
function downloadErrorReport() {
  const name = props.isSuperAdmin
    ? "acceptance.downloadErrorReport.admin"
    : "acceptance.downloadErrorReport";
  const params = props.isSuperAdmin ? {} : { tenantSlug: props.tenantSlug };
